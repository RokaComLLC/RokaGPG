  ...
  if (!no_warning)
    {
      ...
      fprintf(stderr,"please see http://www.gnupg.org/documentation/faqs.html for more information\n");
    }
}
...
# endif /*_AIX*/
     ...
     */
#else
    fprintf(stderr,"Please note that you don't have secure memory on this system\n");
#endif
}
...
init_pool( size_t n)
{
    ...
    if( disable_secmem )
	fprintf(stderr,"secure memory is disabled");
	...
	if( !pool )
	    fprintf(stderr,"can't allocate memory pool of %u bytes\n",(unsigned)poolsize);
	else
	    pool_okay = 1;
    }
...
secmem_init( size_t n )
{
	...
	if( uid != geteuid() ) {
	    if( setuid( uid ) || getuid() != geteuid() || !setuid(0) )
		fprintf(stderr,"failed to drop setuid\n" );
	}
	...
	if( !pool_okay )
	    ...
	    fprintf(stderr,"Oops, secure memory pool already initialized\n");
    }
...
secmem_malloc( size_t size )
{
    ...
    if( !pool_okay ) {
        fprintf(stderr,"operation is not possible without initialized secure memory\n");
        fprintf(stderr,"(you may have used the wrong program for this task)\n");
        exit(2);
    }
...
secmexrealloc( void *p, size_t newsize )
{
    ...
    if (size < sizeof(MEMBLOCK))
     fprintf(stderr,"secure memory corrupted at block %p\n", (void *)mb);
