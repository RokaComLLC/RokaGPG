...
/****************
 * Encrypt the string with the given userids 
 *
 * Memory returned from this must be freed
 */
char *
encode_crypt_str( const char *buffer, STRLIST remusr, int use_symkey )
{
    ...
    /* prepare iobufs */
    //inp = iobuf_open(filename);
    inp = iobuf_temp_with_content(buffer,strlen(buffer) + 1);
    if (inp)
        iobuf_ioctl (inp,3,1,NULL); /* disable fd caching */
    if (inp && is_secured_file (iobuf_get_fd (inp)))
    {
   ...
   //ELMO handle_progress (&pfx, inp, filename);
    
    if( opt.textmode )
        iobuf_push_filter( inp, text_filter, &tfx );
    
    //ELMO if( (rc = open_outfile( filename, opt.armor? 1:0, &out )) )
    //ELMO    goto leave;
...
leave:
    ...
    //Roka Allocate and populate the string for the return value
    RetVal = malloc(out->d.len + 1);
    iobuf_temp_to_buffer(out, RetVal, out->d.len);
 ...
 */
char *
encode_crypt_str2( const char *filename, STRLIST remusr, int use_symkey )
{
    char *RetVal = NULL;
    IOBUF inp = NULL, out = NULL;
    PACKET pkt;
    ...
    if (!opt.no_literal) {
        ...
        if( (rc = build_packet( out, &pkt )) ){
            fprintf(stderr,"build_packet failed: %s\n", g10_errstr(rc) );
        }
        //Roka Allocate and populate the string for the return value
        int sizeToCopy = strlen(pkt.pkt.pubkey_enc->data) + 1;
        RetVal = malloc( sizeToCopy);
        memcpy(RetVal,pkt.pkt.pubkey_enc->data,sizeToCopy);
       // iobuf_read(out, RetVal, out->d.len);
        ...
        while ((bytes_copied = iobuf_read(inp, copy_buffer, 4096)) != -1)
            if (iobuf_write(out, copy_buffer, bytes_copied) == -1) {
                rc = G10ERR_WRITE_FILE;
                fprintf(stderr,"copying input to output failed: %s\n",
                        g10_errstr(rc) );
                break;
            }
        wipememory(copy_buffer, 4096); /* burn buffer */
