...
char * encode_crypt_str( const char *buffer, STRLIST remusr, int use_symkey );
char * encode_crypt_str2( const char *filename, STRLIST remusr, int use_symkey );
...
char * export_pubkeys_as_string( STRLIST users, unsigned int options );
char * export_seckeys_as_string( STRLIST users );
