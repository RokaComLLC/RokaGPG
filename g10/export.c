...
//ROKA - added this to get a char * of requested pubkeys vs standard out
/****************
 * Export the public keys to a char *
 * Depending on opt.armor the output is armored.
 * options are defined in main.h.
 * If USERS is NULL, the complete ring will be exported.  */
char *
export_pubkeys_as_string( STRLIST users, unsigned int options )
{
    char *RetVal = NULL;
    ...
    rc = export_pubkeys_stream(buffer, users, &block, options);
    if(rc == -1){
        goto exit;
 ...
 //   goto exit;
    ...
    fprintf(stderr,"INFO %08lX%08lX BEGIN\n",
    ...
    for(node=block;node;node=node->next)
    {
        ...
        switch(node->pkt->pkttype)
        {
            ...
            case PKT_PUBLIC_SUBKEY:
            {
                ...
                fprintf(stderr,"%sb:%08lX%08lX:%u:%u:%u:%u:",
            ...
            case PKT_SIGNATURE:
            {
                ...
                fprintf(stderr,"sig:%08lX%08lX:%X:%u:%u\n",
    ...
    fprintf(stderr,"INFO %08lX%08lX END\n",
            (ulong)block->pkt->pkt.public_key->keyid[0],
            (ulong)block->pkt->pkt.public_key->keyid[1]);
    
    fprintf(stderr,"KEY %08lX%08lX BEGIN\n",
            (ulong)block->pkt->pkt.public_key->keyid[0],
            (ulong)block->pkt->pkt.public_key->keyid[1]);
    fwrite(iobuf_get_temp_buffer(buffer),
           iobuf_get_temp_length(buffer),1,stderr);
    fprintf(stderr,"KEY %08lX%08lX END\n",
            (ulong)block->pkt->pkt.public_key->keyid[0],
            (ulong)block->pkt->pkt.public_key->keyid[1]);
...
exit:
    ...
    //return do_export( users, 0, options );
    return iobuf_get_temp_buffer(buffer);
#endif
